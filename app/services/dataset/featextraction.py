...
class FeatureExtraction:
    ...
    @staticmethod
    def extract_hog_features(image, pixels_per_cell=(8, 8), cells_per_block=(2, 2), orientations=9, count_feat=True, max_features=600):
    ...
    @staticmethod
    def extract_sift_features(image, n_keypoints=500, contrast_threshold=0.04, edge_threshold=10, count_feat=False, max_features=600):
        ...
        if descriptors is not None:
            # Ensure we don't exceed max_features / 128 (since each descriptor has 128 values)
            max_keypoints = max_features // 128
    ...
    def extract_orb_features(image, n_keypoints=500, scale_factor=1.2, n_level=8, count_feat=False, max_features=600):
        ...
        if descriptors is not None:
            # Limit number of keypoints before flattening
            max_keypoints = max_features // 32  # Each ORB descriptor has 32 values
            descriptors = descriptors[:max_keypoints]
